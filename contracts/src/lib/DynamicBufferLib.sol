...
library DynamicBufferLib {
    ...
    function append(DynamicBuffer memory buffer, bytes memory data) internal pure returns (DynamicBuffer memory) {
        ...
        assembly {
            if mload(data) {
                ...
                for {} iszero(lt(newBufferDataLength, capacity)) {} {
                    ...
                    let newCapacity := and(add(capacity, add(or(capacity, newBufferDataLength), 0x20)), w)
    ...
    /// Returns the same buffer, so that it can be used for function chaining.
    function append(DynamicBuffer memory buffer, bytes memory data0, bytes memory data1, bytes memory data2)
        internal
        pure
        returns (DynamicBuffer memory)
    {
